...
public static class PreventUseHotbarItemNearSignChest
{
    static bool Prefix(Player __instance, int index)
    {
        ...
        if (!Physics.Raycast(cam.transform.position, cam.transform.forward, out var hit, 7f))
            return true;
        ...
        var prefabName = item.m_dropPrefab != null ? item.m_dropPrefab.name : item.m_shared.m_name;
